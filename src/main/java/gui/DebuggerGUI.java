...
public class DebuggerGUI extends JFrame {
    ...
    // Callback pour exécuter les commandes
    ...
    private void initComponents() {
        ...
        // Panel principal avec split vertical
        JSplitPane mainSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
        mainSplit.setDividerLocation(700);

        // Partie gauche : Source code + contrôles
        JPanel leftPanel = createLeftPanel();

        // Partie droite : Call stack + Inspector + Output
    ...
    private void showBreakpointMenu(MouseEvent e) {
        // Cette méthode n'est plus utilisée car le SourceCodePanel gère les breakpoints
    }

    private JPanel createRightPanel() {
        JPanel panel = new JPanel(new BorderLayout(5, 5));

        // Split vertical pour les 3 panels
        JSplitPane topBottomSplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT);

        // Top: Call Stack + Inspector
    ...
    private JPanel createCallStackPanel() {
        ...
        // Clic sur une frame met à jour l'inspector
    ...
    private void updateCallStack() {
        ...
        if (stack != null) {
            ...
            // Sélectionner la première frame par défaut
            if (!frames.isEmpty()) {
    ...
    private void updateInspectorForFrame(int frameIndex) {
        ...
        if (stack != null && frameIndex < stack.getFrames().size()) {
            ...
            for (Variable var : frame.getTemporaries()) {
                addVariableToTree(localsNode, var);
            }
            inspectorRoot.add(localsNode);

            // Ajouter this si disponible
            ObjectReference receiver = frame.getReceiver();
            if (receiver != null) {
                DefaultMutableTreeNode thisNode = new DefaultMutableTreeNode("this");
                addObjectToTree(thisNode, receiver);
                inspectorRoot.add(thisNode);
            }
        }

        inspectorTreeModel.reload();
        expandTree(inspectorTree, 2); // Expand 2 niveaux
    }
    ...
    private void addVariableToTree(DefaultMutableTreeNode parent, Variable var) {
        ...
        // Si c'est un objet, ajouter ses champs
    ...
    private void addObjectToTree(DefaultMutableTreeNode parent, ObjectReference obj) {
        try {
            ...
            for (Field field : refType.allFields()) {
                ...
                // Récursif pour les objets imbriqués (limité à 3 niveaux)
